...
exports.verification = async (req, res) => {
    ...
    try {
        ...
        // Find user
        const user = await db.user.findOne({ _id: decoded.id, email: decoded.email });
...
exports.resetPassword = async (req, res) => {
    ...
    try {
        ...
        const user = await db.user.findOne({ _id: decoded.id, username: decoded.username });
        if (!user) {
...
exports.logout = async (req, res) => {
    try {
        ...
        await db.session.updateOne({ token: token, userId: userId }, { flag: true });
