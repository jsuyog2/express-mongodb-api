...
module.exports = {
    ...
    storeActivity: function (req, cid, activity) {
        ...
        const activitytb = db.collection('activity')
        activitytb.insertMany([{
            c_id: cid,
            login_ip: ip,
            activity: activity,
    ...
    getUserData: function (clienttable, id) {
        return new Promise(function (resolve, reject) {
            clienttable.find({
                "_id": new ObjectID(id)
            },{ projection: { salt: 0, hash: 0 } }).toArray(function (err, result) {
                if (err) {
                    reject({
                        "statusCode": 500,
